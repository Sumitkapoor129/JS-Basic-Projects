...
form.addEventListener("submit",(e)=>{
    if(passhead.children[1]){
        passhead.children[1].remove()
        }
    if(confpasshead.children[1]){
        confpasshead.children[1].remove()
        }
    if(!emailValidation.test(email.value)){
        if(emailhead.children[1]){
            emailhead.children[1].remove()
        }
        warning("Enter a valid Email Address",emailhead)
        e.preventDefault();
    }
    if(!contactValidarion.test(contact.value)){
         if(contacthead.children[1]){
            contacthead.children[1].remove()
